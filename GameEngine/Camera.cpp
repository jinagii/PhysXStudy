...
void Camera::Start()
{
	...
	//LookAtTarget({ 30.0f,0.0f, 30.0f });
	//LookAtVector({ 20.0f, 0.0f, 20.0f });
...
void Camera::Update(float dTime)
{
	...
	m_pRenderer->SetEyePos(m_Transform->GetWorldPosition());
}
...
void Camera::SaveData()
{
	...
	m_SaveData->m_Look_x = m_Look.x;
	m_SaveData->m_Look_y = m_Look.y;
	m_SaveData->m_Look_z = m_Look.z;
...
void Camera::LoadData()
{
	...
	m_Look.x = m_SaveData->m_Look_x;
	m_Look.y = m_SaveData->m_Look_y;
	m_Look.z = m_SaveData->m_Look_z;
